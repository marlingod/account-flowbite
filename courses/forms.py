...
class CourseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'slug': forms.HiddenInput(),  # Hide the slug field as it will be auto-generated
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Field('title'),
            Field('slug'),  # Keep it in the form but hidden
            Field('category'),
            Field('description', rows=4),
    ...
    def clean_slug(self):
        ...
        if not slug and title:
            ...
            # If this is an existing course being edited, exclude it from the check
            if self.instance and self.instance.pk:
                slug_exists = Course.objects.filter(slug=slug).exclude(pk=self.instance.pk).exists()
